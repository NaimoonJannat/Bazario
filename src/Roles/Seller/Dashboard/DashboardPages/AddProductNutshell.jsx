...
const generateDescription = async () => {
  if (!formData.title || !formData.category || !formData.price) {
    Swal.fire("Warning", "Please fill Title, Category and Price first", "warning");
    return;
  }

  setDescLoading(true);
  try {
    const prompt = `Generate an attractive e-commerce product description for:
          ...
          <div className="bg-white p-6 rounded-2xl shadow-md">
            ...
            <div className="space-y-4">
              <div>
                ...
                className="w-full text-[#001f3f] border rounded-lg px-4 py-2" placeholder="Product name"
                 />
              </div>
                ...
                <label className="block mb-1 font-medium text-[#001f3f]">Description</label>
                 {/* AI button  */}
                             <button
                                ...
                                className="flex items-center text-sm bg-[#0077b6] text-white px-3 py-1 rounded hover:bg-[#03045e]"
                              >
                                <FaMagic className="mr-1" /> {descLoading ? "Generating..." : "AI Generate"}
                              </button>
                <textarea required name="description" value={formData.description}
  onChange={handleChange} className="w-full border text-[#001f3f] rounded-lg px-4 py-2" rows="5" placeholder="Product description"></textarea>
              </div>
...
/>
              ...
              <div>
                ...
                <input required type="text" name="price" value={formData.price}
  onChange={handleChange} className="w-full text-[#001f3f] border rounded-lg px-4 py-2" placeholder="$0.00" />
              </div>
